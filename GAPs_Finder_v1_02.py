...
def yfinance_func(nome_ticker):
    ...
    if os.path.exists(cache_file):
        ...
        dati_storici = ticker.history(period="max",auto_adjust=True,timeout = 30)  # dati periodo massimo disponibile
...
with col1:
    ...
    with st.form(key=f'GAPs_Finder'):
         nome_ticker = st.text_input('**GAPs Finder v1.02**',placeholder='Enter the Ticker').strip()
    ...
    if bottone_ricerca:
        ...
        if nome_ticker:
            ...
            dati_yfinance,lunghezza = yfinance_func(nome_ticker)
